...
class DetailedProgressPlugin(octoprint.plugin.EventHandlerPlugin,
    ...
    def do_work(self):
        ...
        try:
            ...
            self._printer.commands("M73 P{}".format(int(currentData["progress"]["printTimeCompletion"])))
    ...
    def _sanitize_current_data(self, currentData):
        ...
        currentData["progress"]["printTimeCompletion"] = 0.00
        ...
        try:
            ...
            currentData["progress"]["printTimeCompletion"] = currentData["progress"]["printTime"] * 100.00 / (currentData["progress"]["printTime"] + currentData["progress"]["printTimeLeft"])
            currentData["progress"]["ETA"] = time.strftime(self._eta_strftime, time.localtime(time.time() + currentData["progress"]["printTimeLeft"]))

        except Exception as e:
            self._logger.info("Caught an exception trying to parse data: {0}\n Error is: {1}\nTraceback:{2}".format(currentData,e,traceback.format_exc()))
    ...
    def _get_next_message(self, currentData):
        ...
        return message.format(
            ...
            printTimeCompletion = currentData["progress"]["printTimeCompletion"],
